...
const scanLargeFiles = (dirPaths, minSizeMB) => {
  return new Promise((resolve, reject) => {
    ...
    const validPaths = dirPaths.filter(p => fs.existsSync(p)).map(p => `"${p}"`).join(' ');
    if (!validPaths) return resolve([]);
    
    // stat -f "%z|%N" prints: bytes|filepath
    const cmd = `find ${validPaths} -type f -size +${minSizeMB}M -exec stat -f "%z|%N" {} +`;
